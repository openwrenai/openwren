...
/**
 * Format the current time in a specific timezone as "YYYY-MM-DD-HHMMSS".
 * Used for slug generation — human-readable, sortable, unique to the second.
 */
export function slugTimestamp(timezone: string): string {
  const formatter = new Intl.DateTimeFormat("en-GB", {
    timeZone: timezone,
    year: "numeric",
    month: "2-digit",
